...
Expression IfThenElseEliminator::VisitIntegral(const Expression& e,
                                               const Formula& guard) {
  return integral(get_integration_var(e),
                  Visit(get_left_endpoint(e), guard),
                  Visit(get_right_endpoint(e), guard),
                  Visit(get_integrand(e), guard));
}
