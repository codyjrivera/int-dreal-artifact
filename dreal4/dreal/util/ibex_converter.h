...
class IbexConverter {
 ...
 private:
  ...
  const ibex::ExprNode* VisitIntegral(const Expression&);
  ...
  // Map stack, for new integral scopes.
  std::vector<std::unordered_map<Variable::Id, const ibex::ExprSymbol*>>
      symbolic_to_ibex_stack_;
