...
IbexConverter::~IbexConverter() {
  ...
  if (need_to_delete_variables_) {
    ...
    for (const auto& a : symbolic_to_ibex_stack_) {
      for (const auto& p : a) {
        delete p.second;
      } 
    }
...
const ExprNode* IbexConverter::VisitIntegral(const Expression& e) {
  //std::cout << e << std::endl;
  //std::cout << e.GetVariables() << std::endl;

  // Visit endpoints
  const ExprNode* left_endpoint{Visit(get_left_endpoint(e))};
  const ExprNode* right_endpoint{Visit(get_right_endpoint(e))};

  // Set up passed-in variables
  ibex::Array<const ExprNode> aux_args(static_cast<int>(e.GetVariables().size()));
  int index = 0;
  for (const Variable& var : e.GetVariables()) {
    aux_args.set_ref(index, *(symbolic_var_to_ibex_var_[var.get_id()]));
    ++index;
  }

  // Set up new environment and function args
  ibex::Array<const ibex::ExprSymbol> fun_args(static_cast<int>(e.GetVariables().size()) + 1);
  std::unordered_map<Variable::Id, const ibex::ExprSymbol*>
      new_scope;
  const Variable& var = get_integration_var(e);
  const ibex::ExprSymbol* ibex_var{
        &ibex::ExprSymbol::new_(var.get_name().c_str(), ibex::Dim::scalar())};
  new_scope.emplace(var.get_id(), ibex_var);
  fun_args.set_ref(0, *ibex_var);
  index = 1;
  for (const Variable& v : e.GetVariables()) {
    const ibex::ExprSymbol* ibex_v{
          &ibex::ExprSymbol::new_(v.get_name().c_str(), ibex::Dim::scalar())};
    new_scope.emplace(v.get_id(), ibex_v);
    fun_args.set_ref(index, *ibex_v);
    ++index;
  }

  // Emplace new environment
  symbolic_to_ibex_stack_.push_back(symbolic_var_to_ibex_var_);
  symbolic_var_to_ibex_var_ = new_scope;

  const ExprNode* body{Visit(get_integrand(e))};

  symbolic_var_to_ibex_var_ = symbolic_to_ibex_stack_.back();
  symbolic_to_ibex_stack_.pop_back();

  const ibex::Function* integrand = new ibex::Function(fun_args, *body);
  
  const ExprNode* n = &integral(*integrand, *left_endpoint, *right_endpoint, aux_args);
  //std::cout << *n << std::endl;
  return n;
}
