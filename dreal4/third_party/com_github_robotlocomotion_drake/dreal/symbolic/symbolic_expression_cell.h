...
/** Symbolic expression representing if-then-else expression.  */
class ExpressionIntegral : public ExpressionCell {
 public:
  /** Constructs integration expression from @p var, @p a, @p b,
   and @p integrand. */
  ExpressionIntegral(const Variable& var,
                     const Expression& a, 
                     const Expression& b,
  ...
  /** Returns the variable of integration */
  const Variable& get_integration_var() const { return var_; }
  /** Returns the left endpoint */
  const Expression& get_left_endpoint() const { return a_; }
  /** Returns the right endpoint */
  const Expression& get_right_endpoint() const { return b_; }
  /** Returns the integrand */
  const Expression& get_integrand() const { return integrand_; }

 private:
  static Variables ExtractVariables(const Variable& var,
                                    const Expression& a,
                                    const Expression& b,
                                    const Expression& integrand);
...
/** Casts @p expr_ptr of const ExpressionCell* to `const
 * ExpressionIntegral*`.
 *  @pre @c *expr_ptr is of @c ExpressionIntegral.
 */
const ExpressionIntegral* to_integral(
    const ExpressionCell* expr_ptr);
/** Casts @p e of Expression to `const ExpressionIntegral`.
 * @pre @c *(e.ptr_) is of @c ExpressionIntegral.
 */
const ExpressionIntegral* to_integral(
    const Expression& e);
