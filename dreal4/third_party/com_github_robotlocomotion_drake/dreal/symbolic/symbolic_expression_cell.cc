...
// ExpressionIntegral
// --------------------
ExpressionIntegral::ExpressionIntegral(const Variable& var,
                                       const Expression& a, 
                                       const Expression& b,
                                       const Expression& integrand)
    : ExpressionCell{ExpressionKind::Integral,
                     hash_combine(hash_value<Variable>{}(var), a,
                                  b, integrand),
                     false, true, ExtractVariables(var, a, b, integrand)},
      var_{var},
      a_{a},
      b_{b},
      integrand_{integrand} {}

Variables ExpressionIntegral::ExtractVariables(const Variable& var,
                           const Expression& a,
                           const Expression& b,
                           const Expression& integrand) {
  Variables ret{a.GetVariables()};
  ret.insert(b.GetVariables());
  // (cody) Exclude the bound variable of integration
  Variables int_vars{integrand.GetVariables()};
  int_vars.erase(var);
  ret.insert(int_vars);
  return ret;
}

bool ExpressionIntegral::EqualTo(const ExpressionCell& e) const {
  // Expression::EqualTo guarantees the following assertion.
  assert(get_kind() == e.get_kind());
  const ExpressionIntegral& int_e{
      static_cast<const ExpressionIntegral&>(e)};
  return var_.equal_to(int_e.var_) && a_.EqualTo(int_e.a_)
      && b_.EqualTo(int_e.b_) && integrand_.EqualTo(int_e.integrand_);
}

bool ExpressionIntegral::Less(const ExpressionCell& e) const {
  // Expression::Less guarantees the following assertion.
  assert(get_kind() == e.get_kind());
  const ExpressionIntegral& int_e{
      static_cast<const ExpressionIntegral&>(e)};
  if (var_.less(int_e.var_)) {
    return true;
  }
  if (int_e.var_.less(var_)) {
    return false;
  }
  if (a_.Less(int_e.a_)) {
    return true;
  ...
  return integrand_.Less(int_e.integrand_);
}

double ExpressionIntegral::Evaluate(const Environment& env) const {
  throw runtime_error("Not yet implemented. (Integral Evaluate)");
}

Expression ExpressionIntegral::Expand() {
  throw runtime_error("Not yet implemented. (Integral Expand)");
}

Expression ExpressionIntegral::Substitute(
    const ExpressionSubstitution& expr_subst,
    const FormulaSubstitution& formula_subst) {
  const Expression a_subst{a_.Substitute(expr_subst, formula_subst)};
  const Expression b_subst{a_.Substitute(expr_subst, formula_subst)};
  // Deal with bound variable in integrand
  ExpressionSubstitution integrand_expr_subst = expr_subst;
  FormulaSubstitution integrand_formula_subst = formula_subst;
  integrand_expr_subst.erase(var_);
  integrand_formula_subst.erase(var_);
  const Expression integrand_subst{integrand_.Substitute(integrand_expr_subst, 
                                                         integrand_formula_subst)};
  if (!a_.EqualTo(a_subst) || !b_.EqualTo(b_subst) ||
      !integrand_.EqualTo(integrand_subst)) {
    return integral(var_, a_subst, b_subst, integrand_subst);
  } else {
    return GetExpression();
  }
}

Expression ExpressionIntegral::Differentiate(const Variable& x) const {
  if (GetVariables().include(x)) {
    ostringstream oss;
    Display(oss) << "is not differentiable with respect to " << x << ".";
    throw runtime_error(oss.str());
...
ostream& ExpressionIntegral::Display(ostream& os) const {
  return os << "(integral_{" << a_ << "}^{" << b_ << "} " << integrand_
            << " d{" << var_ << "})";
}
...
bool is_integral(const ExpressionCell& c) {
  return c.get_kind() == ExpressionKind::Integral;
}
...
const ExpressionIntegral* to_integral(
    const ExpressionCell* const expr_ptr) {
  assert(is_integral(*expr_ptr));
  return static_cast<const ExpressionIntegral*>(expr_ptr);
}
const ExpressionIntegral* to_integral(
    const Expression& e) {
  return to_integral(e.ptr_);
}
