...
#
# Benchmarking script for generating results for Figure 4.
...
# Benchmark list
benchmarks = ["integral_width_triple/svt_gauss_00_eps_0.10_" + eps_upper + "_k_" + str(k)
              for eps_upper in ["0.30", "0.50", "0.70", "0.90"]
              for k in [1, 2, 3, 4]] \
            + ["integral_width_double/svt_product_eps_0.10_" + eps_upper + "_k_" + str(k)
              for eps_upper in ["0.30", "0.50", "0.70", "0.90"]
              for k in [8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48]]
...
with open(output_path + '/figure_4_results.csv', 'w', newline='') as outfile:
