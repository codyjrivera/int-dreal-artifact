...
public:
	ExprTypeMap() {
		...
		insert_new(typeid(ExprIntegral).hash_code(), ExprNode::NumExprIntegral);
	}
};
...
ExprIntegral::ExprIntegral(
	const Function& integrand,
	const Array<const ExprNode>& args
) : 	ExprNAryOp(args, Dim()),
		integrand(integrand) {
	if (integrand.nb_arg() < 1) throw DimException("\"integral\" expects integrand with at least one argument");
	for (const ExprNode& n : args) {
		if (!(n.type() == Dim::SCALAR)) {
			throw DimException("\"integral\" expects scalar arguments");
		}
	}
}
