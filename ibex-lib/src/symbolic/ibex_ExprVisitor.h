...
protected:
   ...
   * By default call visit(const ExprNAryOp&). */
   virtual T visit(const ExprIntegral& e, ArgTypes... args) {
	  return visit((const ExprNAryOp&) e, args...);
  }
...
T ExprNode::accept_visitor(ExprVisitor<T,ArgTypes...>& v, ArgTypes&&... args) const {
  ...
  case NumExprIntegral:			return v.visit((const ExprIntegral&) *this, args...); break;
