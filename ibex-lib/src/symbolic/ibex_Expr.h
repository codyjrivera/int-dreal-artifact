...
public:
	...
	/* For implementation of our own vtable...
	 * (compilers refuse with templates) */
	typedef enum {
		NumExprIndex,
		NumExprSymbol,
		NumExprConstant,
 ...
 * \brief Integral from 
 */
class ExprIntegral : public ExprNAryOp {
public:
	const Function& integrand;

	/** Create an equality constraint [integral]=expr. */
	const ExprCtr& operator=(const ExprNode& expr) const { return ((ExprNode&) *this)=expr; }

	/** Create an equality constraint [integral]=value. */
	const ExprCtr& operator=(const Interval& value) const  { return ((ExprNode&) *this)=value; }

	static const ExprIntegral& new_(
		const Function& integrand,
		const ExprNode& a,
		const ExprNode& b,
		const Array<const ExprNode>& aux
	) {
		Array<const ExprNode> args;
		args.add(a);
		args.add(b);
		args.add(aux);
		return *new ExprIntegral(integrand, args);
	}
...
private:
	...
	ExprIntegral(const ExprIntegral&); // copy constructor forbidden
};
...
/** Application */
inline const ExprApply& apply(const Function& func, const Array<const ExprNode>& args) {
	return ExprApply::new_(func,args);
}
...
/** Integral */
inline const ExprIntegral& integral(
	const Function& integrand,
	const ExprNode& a,
	const ExprNode& b,
	const Array<const ExprNode>& aux = Array<const ExprNode>()
)
{
	return ExprIntegral::new_(integrand, a, b, aux);
}
