...
void ExprLinearity::visit(const ExprSymbol& e) { 
	insert_coeff_and_check(e, build_cst(Domain(e.dim)) /* (-oo,oo) */);
}
...
void ExprLinearity::visit(const ExprIntegral& e) {
	Array<Domain>* d=new Array<Domain>(n+1);

	for (const ExprNode& arg : e.args) {
		visit(arg);
	}
