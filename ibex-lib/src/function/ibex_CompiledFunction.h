...
#pragma clang diagnostic pop
	...
	void visit(const ExprIntegral& e);
...
void CompiledFunction::forward(const V& algo, int i) const {
	...
	case INTEGRAL:  ((V&) algo).integral_fwd  (args[i],i); break;
...
void CompiledFunction::backward(const V& algo, int i) const {
	...
	case INTEGRAL:  ((V&) algo).integral_bwd  (args[i],i); break;
	default: 	 assert(false);
	}
}
