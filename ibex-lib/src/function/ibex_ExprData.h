...
public:
	 ...
	 * By default call init(const ExprUnaryOp&). */
	virtual T* init(const ExprSaw& e, T& expr_deco) {
		return init((const ExprUnaryOp&) e, expr_deco);
	}

	/*==================== integral =========================*/
	/** Visit an integral (Implementation is not mandatory).
	 * By default call init(const ExprNAryOp&). */
	virtual T* init(const ExprIntegral& e, Array<T>& args_deco) {
		return init((const ExprNAryOp&) e, args_deco);
	}
...
//private:
	...
	virtual void visit(const ExprSaw& e)             { __visit_unary<ExprSaw>(e); }
	virtual void visit(const ExprIntegral& e)        { __visit_nary<ExprIntegral>(e); }
