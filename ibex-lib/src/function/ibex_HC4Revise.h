...
//	bool proj(const Domain& y, const Array<const Domain>& x);
	...
	/**
	 * Evaluates int_{d2[0]}^{d2[1]} n.integrand(x0, ...) dx0.
	 * (evaluates the integral that's given with the arguments
	 * that are given)
	 */
	Interval eval_integral(const ExprIntegral& n, const Array<Domain>& d2);
